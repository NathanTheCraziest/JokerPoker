...
func _on_card_scored(card: CardInstance):
	if card.suit == CardData.Suit.HEART or card.enhancement == CardData.Enhancement.WILD and card.enhancement != CardData.Enhancement.STONE:
...
[color=red]+4 Mult[/color][color=gray] for each Heart card scored[/color][/center]
"
